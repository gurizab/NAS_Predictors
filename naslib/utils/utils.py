...
def get_config_from_file_path(config_file=None, config_type='nas'):
    """
    ...
    Args:
        config_file: File path to the config file.
    """
    ...
    # Override file args with ones from config file path
    # load config file
...
def cross_validation(xtrain, ytrain, train_info, predictor, split_indices, score_metric='kendalltau'):
    ...
    for train_indices, validation_indices in split_indices:
        ...
        train_info_i = [train_info[j] for j in train_indices]
        val_info_i = [train_info[j] for j in validation_indices]
    ...
    score = np.mean(validation_score) 
    if np.isnan(score) and score_metric == 'kendalltau':
            score = np.nanmean(validation_score)
            if np.isnan(score):
                score = 0
    return score
