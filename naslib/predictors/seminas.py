...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.rnn = nn.LSTM(self.hidden_size, self.hidden_size, self.layers, batch_first=True, dropout=dropout, bidirectional=True).to(device)
        self.out_proj = nn.Linear(self.hidden_size * 2, self.hidden_size, bias=True).to(device)
        self.mlp = nn.ModuleList([]).to(device)
...
class NAO(nn.Module):
    ...
    def generate_new_arch(self, input_variable, predict_lambda=1, direction='-'):
        encoder_outputs, encoder_hidden, arch_emb, predict_value, new_encoder_outputs, new_arch_emb, new_predict_value = self.encoder.infer(
            input_variable.to(device), predict_lambda, direction=direction)
        new_encoder_hidden = (new_arch_emb.unsqueeze(0), new_arch_emb.unsqueeze(0))
        decoder_outputs, new_archs = self.decoder(None, new_encoder_hidden, new_encoder_outputs.to(device))
