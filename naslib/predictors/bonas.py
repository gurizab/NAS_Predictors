...
class BonasPredictor(Predictor):
    def __init__(self, encoding_type='bonas', ss_type=None, hpo_wrapper=False, hyperparams=None, n_hypers=30):
    ...
    def train(self, xtrain, ytrain, model, params, epochs=100, wd=0):
        data_loader = DataLoader(xtrain, batch_size=params['batch_size'], shuffle=True, drop_last=False)
        model.to(device)
        criterion = nn.MSELoss().to(device)
        optimizer = optim.Adam(model.parameters(), lr=params['lr'], weight_decay=wd)
        ...
        for _ in range(epochs):
            ...
            lr_scheduler.step()
        train_pred = np.squeeze(self.hpo_query(model, xtrain))
    ...
    def fit(self, xtrain, ytrain, train_info=None,
        ...
        for _ in range(epochs):
            ...
            lr_scheduler.step()
        train_pred = np.squeeze(self.query(xtrain, omni=omni))
        train_error = np.mean(abs(train_pred-ytrain))
        return train_error
