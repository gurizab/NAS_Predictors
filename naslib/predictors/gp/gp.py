...
class GPPredictor(BaseGPModel):
    ...
    def __init__(self, encoding_type='adjacency_one_hot',
                 ...
                 zc=False, lengthscale=10, hpo_wrapper=False):
        """
        Params:
        ...
        super(GPPredictor, self).__init__(encoding_type, ss_type, kernel_type,
                                          optimize_gp_hyper, hpo_wrapper)
        self.kernel = partial(eval('gp.kernels.'+kernel_type),
                              variance=torch.tensor(5.).double(),
                              lengthscale=torch.tensor(lengthscale).double())
        self.hpo_wrapper = hpo_wrapper

    def set_hyperparams(self, hyperparams):
        self.kernel = partial(eval('gp.kernels.' + hyperparams['kernel_type']),
                              variance=torch.tensor(5.).double(),
                              lengthscale=torch.tensor(hyperparams['lengthscale']).double())
        self.kernel_type = hyperparams['kernel_type']
